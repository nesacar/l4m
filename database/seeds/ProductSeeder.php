...
class ProductSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        ])->each(function ($p){ $p->category()->sync(1); $p->tag()->sync([2,45,3,7,8]); });
        ...
        ])->each(function ($p){ $p->category()->sync(2); $p->tag()->sync([12,22,13,37,48]); });
        ...
        ])->each(function ($p){ $p->category()->sync(3); $p->tag()->sync([4,44,23,32,48]); });
        ...
        ])->each(function ($p){ $p->category()->sync(4); $p->tag()->sync([6,12,7,32,48]); });
    }
}
