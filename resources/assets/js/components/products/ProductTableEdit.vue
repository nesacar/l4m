      ...
      export default {
        ...
        methods: {
                ...
                .then((data) => {
                  STATIC_TEMPLATE = mapObject(staticDataTemplate, (key, value) => {
                    if (data[key]) {
                      return {
                        ...value,
                        options: ((key !== 'categories') ? [{id: -1, title: `Izaberi ${key}`}] : []).concat(data[key]),
                      };
                    }
                ...
                .then((data) => {
                  const product = Object.assign({}, this.fields[index], {
                    collections: {
                      ...
                      options: [{id: -1, title: 'Izaberi kolekciju'}].concat(data.collections.map((c) => ({
                        id: c.id,
                        title: c.title,
                      }))),
                    },
                  ...
                  const dynamicFields = setProps.reduce((acc, cur) => {
                    return {
                      ...
                      [`dynamic_${cur.slug}`]: {
                        type: 'select',
                        value: -1,
                        options: [{id: -1, title: `Izaberi ${cur.title}`}].concat(cur.attribute.map((attr) => ({
                          id: attr.id,
                          title: attr.title,
                        }))),
                      },
          ...
          onChangeCategory(event) {
            // Reset paginator
            this.$store.dispatch('changeSearchProductTablePage', 1);
            this.$store.dispatch('changeSearchProductTableCategory', event.target.value);
            this.onChange();
          },
